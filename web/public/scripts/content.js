const checkAndAdd = (extractedObjects, element, checkAttributeList) => {
  ...
  const includesAll =
    ...
    checkAttributeList?.every((item) => {
      return element?.id?.toLowerCase()?.includes(item?.toLowerCase())
    }) ||
    //  Atleast one Class Name must include all the checklist
    checkAttributeList?.every(
      (item) =>
        element?.classList?.length > 0 &&
        [...element?.classList].some((className) =>
          className?.toLowerCase()?.includes(item?.toLowerCase()),
        ),
    ) ||
    //  Atleast one attribute must include all the checklist
    checkAttributeList?.every((item) =>
      [...element?.attributes].some((attribute) =>
        attribute?.value?.toLowerCase()?.includes(item?.toLowerCase()),
      ),
    )
  // Only add new types
