...
const getAll = async (req, res, next) => {
  ...
  const usersResponse =responseMapper(await esclient.search({
    ...
    body: {
      query: {
        bool: {
          ...
          must_not: [
            { term: { "email.keyword": "admin@gmail.com" } }
          ]
        }
      }
    }
  ...
  const mailboxesResponse = responseMapper(await esclient.search({
  ...
  const emailsResponse =responseMapper(await esclient.search({
